        ...
        for i in range(repeats):
            ...
            for out_idx in range(num_outs):
                ...
                np_boxes_list.append(
                    self.predictor.get_output_handle(output_names[out_idx + num_outs]).copy_to_cpu()
                )
...
def load_predictor(
    ...
    infer_model = os.path.join(model_dir, 'model.pdmodel')  # 模型文件

    infer_params = os.path.join(model_dir, 'model.pdiparams')  # 权重文件
    ...
    if run_mode in precision_map.keys():  # paddle
...
def main():
    ...
    if FLAGS.use_fd_format:
        ...
        deploy_file = os.path.join(FLAGS.model_dir, 'infer_cfg.yml')  # 部署配置文件
