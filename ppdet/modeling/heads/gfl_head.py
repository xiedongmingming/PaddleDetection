...
class GFLHead(nn.Layer):
    ...
    def get_loss(self, gfl_head_outs, gt_meta):
        ...
        grid_cells_list = self._images_to_levels(gt_meta['grid_cells'], num_level_anchors)

        labels_list = self._images_to_levels(gt_meta['labels'], num_level_anchors)
...
class LDGFLHead(GFLHead):
    ...
    def get_loss(self, gfl_head_outs, gt_meta, soft_label_list, soft_targets_list):
        ...
        # vlr regions                                         
        vlr_regions_list = self._images_to_levels(gt_meta['vlr_regions'], num_level_anchors)
        ...
        for cls_score, bbox_pred, grid_cells, labels, label_weights, bbox_targets, stride, soft_targets, soft_label, vlr_region in zip(
            cls_logits,
            bboxes_reg,
            grid_cells_list,
            labels_list,
            ...
            pos_inds = paddle.nonzero(
                paddle.logical_and((labels >= 0), (labels < bg_class_ind)), as_tuple=False
            ).squeeze(1)
