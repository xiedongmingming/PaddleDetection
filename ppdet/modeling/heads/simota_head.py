...
class OTAHead(GFLHead):
    ...
    def _get_target_single(self, flatten_cls_pred, flatten_center_and_stride, flatten_bbox, gt_bboxes, gt_labels):
        """Compute targets for priors in a single image.
        """
        pos_num, label, label_weight, bbox_target = self.assigner(
            F.sigmoid(flatten_cls_pred),
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        featmap_sizes = [[featmap.shape[-2], featmap.shape[-1]] for featmap in cls_scores]
        ...
        for featmap_size, stride, bbox_pred in zip(featmap_sizes, self.fpn_stride, bbox_preds):
            ...
            yy, xx = self.get_single_level_center_point(featmap_size, stride, self.cell_offset)
            ...
            decode_bbox_pred_wo_stride = distance2bbox(
                center_in_feature, pred_distances
            ).reshape([num_imgs, -1, 4])

            decode_bbox_preds.append(decode_bbox_pred_wo_stride * stride)

        flatten_cls_preds = [
            cls_pred.transpose([0, 2, 3, 1]).reshape([num_imgs, -1, self.cls_out_channels])
        ...
        for cls_score, bbox_pred, center_and_strides, labels, label_weights, bbox_targets, stride in zip(
            ...
            if len(pos_inds) > 0:
                ...
                pos_centers = paddle.gather(
                    center_and_strides[:, :-2], pos_inds, axis=0
                ) / stride
                ...
                loss_bbox = paddle.sum(
                    self.loss_bbox(pos_decode_bbox_pred, pos_decode_bbox_targets) * weight_targets
                )
...
class OTAVFLHead(OTAHead):
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        for featmap_size, stride, bbox_pred in zip(featmap_sizes, self.fpn_stride, bbox_preds):
            ...
            # center in origin image
            yy, xx = self.get_single_level_center_point(
                featmap_size, stride, self.cell_offset
            )

            strides = paddle.full((len(xx), ), stride)

            center_and_stride = paddle.stack(
                [xx, yy, strides, strides], -1).tile([num_imgs, 1, 1]
            )
        ...
        flatten_cls_preds = [
            cls_pred.transpose([0, 2, 3, 1]).reshape([num_imgs, -1, self.cls_out_channels]) for cls_pred in cls_scores
        ]
        ...
        for flatten_cls_pred, flatten_center_and_stride, flatten_bbox,gt_box,gt_label \
                in zip(flatten_cls_preds.detach(), flatten_center_and_strides.detach(), flatten_bboxes.detach(),gt_boxes,gt_labels):
        ...
        for cls_score, bbox_pred, center_and_strides, labels, label_weights, bbox_targets, stride in zip(
                cls_scores, bbox_preds, center_and_strides_list, labels_list, label_weights_list, bbox_targets_list, self.fpn_stride
        ):
            ...
            if len(pos_inds) > 0:
                ...
                pos_decode_bbox_pred = distance2bbox(pos_centers, pos_bbox_pred_corners)
