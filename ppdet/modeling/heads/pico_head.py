    ...
    def _generate_anchors(self, feats=None):
        ...
        for i, stride in enumerate(self.fpn_stride):
            ...
            anchor_point = paddle.cast(
                paddle.stack([shift_x, shift_y], axis=-1), dtype='float32'
            )
...
class PicoHeadV2(GFLHead):
        ...
        for i, (fpn_feat, stride) in enumerate(zip(fpn_feats, self.fpn_stride)):
            ...
            if not export_post_process:
                ...
                cls_score_list.append(
                    cls_score.reshape([1, self.cls_out_channels, -1]).transpose([0, 2, 1])
                )

                box_list.append(
                    reg_pred.reshape([1, (self.reg_max + 1) * 4, -1]).transpose([0, 2, 1])
                )
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        flatten_centers = centers.expand(
            [num_imgs, centers_shape[0], centers_shape[1]]
        ).reshape([-1, 2])

        flatten_strides = stride_tensor_list.expand(
            [num_imgs, centers_shape[0], 1]
        ).reshape([-1, 1])
        ...
        pos_inds = paddle.nonzero(
            paddle.logical_and((flatten_labels >= 0), (flatten_labels < self.num_classes)),
            as_tuple=False
        ).squeeze(1)
        ...
        if num_total_pos > 0:
            ...
            pos_centers = paddle.gather(
                flatten_centers, pos_inds, axis=0
            ) / pos_strides
            ...
            loss_bbox = paddle.sum(
                self.loss_bbox(pos_decode_bbox_pred, pos_bbox_targets) * weight_targets
            )
