...
class Trainer(object):
    ...
    def __init__(self, cfg, mode='train'):
        ...
        assert mode.lower() in ['train', 'eval', 'test'], "mode should be 'train', 'eval' or 'test'"
        ...
        if 'slim' in cfg and cfg['slim_type'] == 'OFA':
            ...
            self.model.student_model.load_meanstd(cfg['TestReader'][
                ...
                'slim_type'
        ] == 'DistillPrune' and self.mode == 'train':
    ...
    def _init_metrics(self, validate=False):
        ...
        if self.cfg.metric == 'COCO' or self.cfg.metric == "SNIPERCOCO":
            ...
            # pass clsid2catid info to metric instance to avoid multiple loading annotation file
    ...
    def register_callbacks(self, callbacks):
        ...
        for c in callbacks:
            ...
            assert isinstance(c, Callback), "metrics shoule be instances of subclass of Metric"
    ...
    def register_metrics(self, metrics):
        ...
        for m in metrics:
            ...
            assert isinstance(m, Metric), "metrics shoule be instances of subclass of Metric"

        self._metrics.extend(metrics)

    def load_weights(self, weights, ARSL_eval=False): # 模型加载权重数据
    ...
    def train(self, validate=False):
        ...
        model = self.model  # PicoDet

        if self.cfg.get('to_static', False):

            model = apply_to_static(self.cfg, model)

        sync_bn = (getattr(self.cfg, 'norm_type', None) == 'sync_bn' and (self.cfg.use_gpu or self.cfg.use_mlu) and self._nranks > 1)
        ...
        if self.cfg.get('fleet', False):
            ...
            ] if 'find_unused_parameters' in self.cfg else False
        ...
        ] if 'use_fused_allreduce_gradients' in self.cfg else False

        for epoch_id in range(self.start_epoch, self.cfg.epoch):  # 分EPOCH训练
            ...
            is_snapshot = (self._nranks < 2 or (self._local_rank == 0 or self.cfg.metric == "Pose3DEval")) and ((epoch_id + 1) % self.cfg.snapshot_epoch == 0 or epoch_id == self.end_epoch - 1)
